...
def password_reset(request):
    """abcd forgot password form view."""
    msg = ''
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if len(qs) > 0:
                user = qs[0]
                user.is_active = False  # abcd needs to be inactive for the reset password duration
...
def view_user(request, username):
    user = Profile.objects.select_related(user__username=username)
